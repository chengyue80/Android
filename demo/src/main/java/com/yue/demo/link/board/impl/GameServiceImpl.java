...
public class GameServiceImpl implements GameService {
	...
	public void start() {
		...
		LogUtil.d(TAG, "GameServiceImpl start");
		// 定义一个AbstractBoard对象
		AbstractBoard board = null;
		int index = new Random().nextInt(4);
		LogUtil.d(TAG, "random index : " + index);
		switch (index) {
		case 0:
			// 0返回VerticalBoard(竖向)
	...
	public Piece[][] getPieces() {
		// TODO Auto-generated method stub
		LogUtil.d(TAG, "GameServiceImpl getPieces");
		return this.pieces;
	}
	...
	public boolean hasPieces() {
		// TODO Auto-generated method stub
		LogUtil.d(TAG, "GameServiceImpl hasPieces");

		for (int i = 0; i < pieces.length; i++) {
			for (int j = 0; j < pieces[i].length; j++) {
				if (pieces[i][j] != null) {

					LogUtil.d(TAG, "GameServiceImpl hasPieces return true");
					return true;
				}
			}
		}
	...
	public Piece findPiece(float touchX, float touchY) {
		// TODO Auto-generated method stub
//		Mlog.d(TAG, "GameServiceImpl findPiece");
		...
		// 第二个参数为每张图片的宽
		int indexX = getIndex(relativeX,
				(int) (GameConf.getImageWidth()));
		int indexY = getIndex(relativeY,
				(int) (GameConf.getImageHeight()));
		// 这两个索引比数组的最小索引还小, 返回null
		// 这两个索引比数组的最大索引还大(或者等于), 返回null
		if (indexX < 0 || indexY < 0 || indexX >= config.getXSize()
				|| indexY >= config.getYSize()) {
			return null;
		}

		LogUtil.i(TAG, "indexX = " + indexX + "  indexY = " + indexY);
		return this.pieces[indexX][indexY];
	}

	/**
	 * 工具方法, 根据relative座标计算相对于Piece[][]数组的第一维或第二维的索引值 ，size为每张图片边的长或者宽
	 * 
	 * @param relative
	 *            点中的坐标
	 * @param size
	...
	public LinkInfo link(Piece p1, Piece p2) {
		// TODO Auto-generated method stub
		LogUtil.d(TAG, "GameServiceImpl link");
		// 两个Piece是同一个, 即选中了同一个方块, 返回null
		// 如果p1的图片与p2的图片不相同, 则返回null
		if (p1.equals(p2) || !(p1.isSameImage(p2)))
			return null;
		...
		if (p1.getIndexY() == p2.getIndexY()) {
			LogUtil.d(TAG, "piece row is the one");
			// 它们在同一行并可以相连
			if (!isXBlock(point1, point2,
					(int) (GameConf.getImageWidth()))) {
		...
		if (p1.getIndexX() == p2.getIndexX()) {
			LogUtil.d(TAG, "piece column is the one");
			if (!isYBlock(point1, point2,
					(int) (GameConf.getImageHeight() ))) {
				// 它们之间没有真接障碍, 没有转折点
				return new LinkInfo(point1, point2);
			}
		...
		Point cornerPoint = getCornerPoint(point1, point2,
				GameConf.getImageWidth(), GameConf.getImageHeight());
		if (cornerPoint != null) {
			LogUtil.d(TAG, "cornerPoint 直角连线");
			return new LinkInfo(point1, cornerPoint, point2);
		}
		// 该map的key存放第一个转折点, value存放第二个转折点,
		// map的size()说明有多少种可以连的方式
		Map<Point, Point> turns = getLinkPoints(point1, point2,
				GameConf.getImageWidth(), GameConf.getImageWidth());
		if (turns.size() != 0) {
			return getShortcut(point1, point2, turns,
	...
	private Point getCornerPoint(Point point1, Point point2, int pieceWidth,
			int pieceHeight) {
		LogUtil.d(TAG, "getCornerPoint w/h : " + pieceWidth + "/" + pieceHeight);
		LogUtil.d(TAG, "point1 x/y : " + point1.x + "/" + point1.y);
		LogUtil.d(TAG, "point2 x/y : " + point2.x + "/" + point2.y);
		// 先判断这两个点的位置关系
