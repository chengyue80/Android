...
public class LogUtil {
	...
	private static final String LOG_PATH = ConstantUtil.LOG_DIR;
	private final static String mTag = "LogUtil";
	...
	public static void a(Class<?> clazz, String method, MotionEvent event,
			String log, boolean ignoreDuplicate) {
		if (!debug || event == null)
			return;
		...
		sb.append(TextUtils.isEmpty(calssName) ? "" : ", method : " + method);
		sb.append(", action : " + getAction(action));
		sb.append(TextUtils.isEmpty(log) ? "" : ", log : " + log);
	...
	public static void k(Class<?> clazz, String method, int keyCode,
			KeyEvent event) {
		k(clazz, method, keyCode, event, "");
	}

	public static void k(Class<?> clazz, String method, int keyCode,
			KeyEvent event, String log) {
		if (!debug) {
			return;
		}
