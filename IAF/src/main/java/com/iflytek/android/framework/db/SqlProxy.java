...
public class SqlProxy {
	...
	public static SqlProxy update(Object obj) {
		...
		proxy.sql.deleteCharAt(proxy.sql.length() - 1);
		proxy.sql.append(" WHERE ").append(entity.getColumns().get(pk))
				.append("=?");
	...
	private void buildWhere(String where, Object[] whereargs) {
		...
		while (matcher.find()) {
			...
			where = where.replace(key, entity.getColumns()
					.get(matcher.group(1)));
		}
		sql.append(where);
		if (whereargs != null) {
	 ...
	 * @return
	 */
	public static SqlProxy update(Class clazz, Map<String, Object> values,
	...
	public static SqlProxy delete(Object obj) {
		...
		proxy.sql.append("DELETE FROM ").append(entity.getTable())
				.append(" WHERE ").append(entity.getColumns().get(pk))
				.append("=?");
		proxy.params.add(BeanUtil.getProperty(obj, pk));
		proxy.clazz = obj.getClass();
		return proxy;
	 ...
	 * @return
	 */
	public static SqlProxy delete(Class clazz, String where,
			Object... whereargs) {
		SqlProxy proxy = new SqlProxy();
		EntityInfo entity = EntityInfo.build(clazz);
		String pk = entity.getPk();
	 ...
	 * @return
	 */
	public static SqlProxy select(Class clazz, String where,
			Object... whereargs) {
		SqlProxy proxy = new SqlProxy();
		EntityInfo entity = EntityInfo.build(clazz);
	...
	public static SqlProxy selectCount(Class clazz, String where,
            Object... whereargs) {
        SqlProxy proxy = new SqlProxy();
        EntityInfo entity = EntityInfo.build(clazz);
        proxy.sql.append("SELECT count(*) FROM ").append(entity.getTable());
        proxy.clazz = clazz;
        proxy.buildWhere(where, whereargs);
        return proxy;
    }
